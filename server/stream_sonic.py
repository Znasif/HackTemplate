...
class AudioStreamRecorder:
    """Handles recording of streaming audio chunks and saves them as an MP3 file."""
    ...
    def _convert_webm_to_mp3(self, webm_data: bytes) -> Optional[bytes]:
        ...
        try:
            cmd = ['ffmpeg', '-i', temp_webm_path, '-acodec', 'libmp3lame', '-b:a', '128k', temp_mp3_path, '-y']
            subprocess.run(cmd, capture_output=True, text=True, check=True)
            with open(temp_mp3_path, 'rb') as f: return f.read()
        except subprocess.CalledProcessError as e:
            log_message(f"FFmpeg conversion failed: {e.stderr}", level="ERROR")
            return None
        finally:
...
class ConnectionManager:
    def __init__(self):
        ...
        self.nova_sessions: Dict[WebSocket, NovaSonicStreamManager] = {}
    ...
    async def connect(self, websocket: WebSocket):
        ...
        self.nova_sessions[websocket] = NovaSonicStreamManager(websocket, self)
