...
class SonicStreamManager:
    ...
    # Event templates (keeping the same as in the original)
    START_SESSION_EVENT = '''{
        "event": {
            "sessionStart": {
    ...
    def _create_prompt_start_event(self) -> str:
        """Create a promptStart event with processor tools"""
        # Schema for processor execution
        execute_processor_schema = json.dumps({
            "type": "object",
            "properties": {
                "processor_id": {
                    "type": "integer",
        ...
        # Schema for listing processors
        list_processors_schema = json.dumps({
            "type": "object",
            "properties": {},
            "required": []
        })
                        ...
                        "tools": [
                            {
                                "toolSpec": {
                                    ...
                                    "description": "Execute a specific processor to handle image/video processing tasks based on the user's request",
                                    "inputSchema": {
                                        "json": execute_processor_schema
                                    }
                                }
                            },
                            {
                                "toolSpec": {
                                    ...
                                    "description": "List all available processors with their descriptions when the user asks what processors are available",
                                    "inputSchema": {
                                        "json": list_processors_schema
                                    }
                                }
                            }
                        ]
...
Keep your responses concise and conversational. Do not ask clarifying questions unless absolutely necessary."""
    ...
    async def _initialize_stream(self, session_data: Dict[str, Any]):
        ...
        # Send initialization events
        prompt_event = self._create_prompt_start_event()
        ...
        text_content = self.TEXT_INPUT_EVENT % (self.prompt_name, self.content_name, system_instruction)
        ...
        for event in init_events:
            await self._send_raw_event(stream, event)
            await asyncio.sleep(0.1)
        
        # Send audio content start
        content_start_event = self.CONTENT_START_EVENT % (self.prompt_name, self.audio_content_name)
        await self._send_raw_event(stream, content_start_event)

    async def _send_raw_event(self, stream, event_json: str):
        """Send a raw event JSON to the Sonic stream."""
        ...
        try:
            ...
            self.log_message(f"Sent event to Sonic")
        except Exception as e:
    ...
    async def send_audio_to_sonic(self, websocket):
        ...
        while session_data['is_active']:
            try:
                ...
                await self._send_raw_event(stream, audio_event)
                self.log_message(f"Sent audio chunk to Sonic")
    ...
    async def _handle_sonic_event(self, websocket, event: Dict[str, Any]):
        ...
        if 'textOutput' in event:
            ...
            tool_name = tool_content['toolName']
            tool_use_id = tool_content['toolUseId']
    ...
    async def _handle_tool_request(self, websocket, tool_name: str, tool_content: Dict, tool_use_id: str):
        ...
        try:
            ...
            if tool_name == "execute_processor":
                # Extract processor ID and reason
                ...
                # Get the last frame if available
                last_frame = self.last_frame_by_websocket.get(websocket)
                if last_frame and self.connection_manager:
                    # Execute the processor
                    result = await self.connection_manager.execute_request(processor_id, last_frame)
                    
                    # Send tool result back to Sonic
                    await self._send_tool_result(websocket, tool_use_id, {
                        "status": "success",
                        "processor_id": processor_id,
                        "result": str(result.get("text", "Processing completed"))
                    })
                else:
                    await self._send_tool_result(websocket, tool_use_id, {
                        "status": "error",
                        "message": "No image frame available for processing"
                    })
    ...
    async def _send_tool_result(self, websocket, tool_use_id: str, result: Dict):
        ...
        tool_start = self.TOOL_CONTENT_START_EVENT % (self.prompt_name, content_name, tool_use_id)
        await self._send_raw_event(stream, tool_start)
        ...
        await self._send_raw_event(stream, json.dumps(tool_result_event))
        
        # End tool content
        tool_end = self.CONTENT_END_EVENT % (self.prompt_name, content_name)
        await self._send_raw_event(stream, tool_end)
    ...
    async def handle_audio_chunk(self, websocket, audio_chunk_b64: str, session_id: str):
        ...
        try:
            ...
            if websocket in self.active_sessions:
                session_data = self.active_sessions[websocket]
                # Sonic expects base64 encoded audio
    ...
    async def stop_session(self, websocket):
        ...
        try:
            ...
            if session_data['sonic_stream']:
                audio_end = self.CONTENT_END_EVENT % (self.prompt_name, self.audio_content_name)
                prompt_end = self.PROMPT_END_EVENT % (self.prompt_name)
                
                await self._send_raw_event(session_data['sonic_stream'], audio_end)
                await self._send_raw_event(session_data['sonic_stream'], prompt_end)
                await self._send_raw_event(session_data['sonic_stream'], self.SESSION_END_EVENT)
