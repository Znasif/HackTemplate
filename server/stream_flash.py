    ...
    async def handle_tool_call(self, websocket: WebSocket, func_call):
        ...
        try:
            ...
            if func_name == "execute_processor":
                ...
                # Notify client to set the processor (as per client-side handleServerResponse)
                await websocket.send_text(json.dumps({
                    "text": "set_processor",
                    "processor_id": processor_id, # Ensure this is an integer
                    "reason": reason
                }))
                self.log_message(f"Processor Handling with Geminis: {processor_id}, Reason: {reason}")
                ...
                self.log_message(f"Processor List Handling with Gemini")
