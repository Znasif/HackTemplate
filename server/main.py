...
PROCESSOR_CONFIG = {
    ...
    # Add more processors here, e.g.:
    # 2: {
    #     "host": "127.0.0.1",
    #     "port": 8002,
    #     "name": "FastVLMProcessor",
    #     "conda_env": "fastvlm_env"
    # },
}
...
async def start_processor_servers():
    ...
    for processor_id, config in PROCESSOR_CONFIG.items():
        ...
        script_path = os.path.join(parent_dir, f"run_{config['name']}.sh")
        script_content = f"""#!/bin/bash
source $(conda info --base)/etc/profile.d/conda.sh
conda activate {config['conda_env']}
cd {parent_dir}
uvicorn new_processors.{config['name']}:app --host {config['host']} --port {config['port']} --log-level debug > {os.path.join(parent_dir, f"{config['name']}.log")} 2>&1
"""
        ...
        try:
            ...
            print(f"Started {config['name']} in {config['conda_env']} on {config['host']}:{config['port']}, logging to {os.path.join(parent_dir, config['name'] + '.log')}")
        except Exception as e:
...
async def shutdown_event():
    ...
    for processor_id, config in PROCESSOR_CONFIG.items():
        script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), f"run_{config['name']}.sh")
